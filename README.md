...
> **v2.1** — 新增云同步功能，支持账号登录、本地与服务器双向同步、增量同步等特性。
...
- **☁️ 云同步** - 账号登录后自动同步书签到云端，多设备同步
- **🔄 双向同步** - 本地与服务器数据双向合并，确保数据一致
...
│   ├── style.css              # 样式备份
│   └── modules/
│       ├── api.js             # API 请求封装、Token 管理
│       ├── sync.js            # 云同步逻辑、双向同步
│       ├── auth.js            # 登录/注册 UI、账号管理
...
- ✅ 新增云同步功能：支持账号注册/登录
- ✅ 双向同步：本地与服务器书签自动合并
- ✅ 增量同步：新增/编辑/删除操作实时同步到云端
- ✅ 登录弹窗：支持登录/注册 Tab 切换
- ✅ 自动登录：记住登录状态，插件启动自动恢复
